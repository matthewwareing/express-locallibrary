...
exports.bookinstance_create_get = function (req, res, next) {
    Book.find({}, 'title')
        .exec(function (err, books) {
            if (err) { return next(err); }
            res.render('bookinstance_form', { title: 'Add Copy of a Book', book_list: books });
        });
};
...
exports.bookinstance_create_post = [
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            // There are errors. Render form again with sanitized value and error msgs
            Book.find({}, 'title')
                .exec(function (err, books) {
                    if (err) { return next(err); }
                    res.render('bookinstance_form', { title: 'Add BookInstance', book_list: books, selected_book: bookinstance.book_id, errors: errors.array(), bookinstance: bookinstance });
                });
            return;
        }
        else {
...
exports.bookinstance_delete_get = function (req, res, next) {
    BookInstance.findById(req.params.id)
        ...
        .exec(function (err, book_instance) {
            ...
            // Successful, so render.
            res.render('bookinstance_delete', { title: 'Delete Book Copy', bookinstance: book_instance });
        });
};
...
exports.bookinstance_delete_post = function (req, res) {
    BookInstance.findByIdAndRemove(req.params.id, function deleteBookInstance(err) {
        if (err) { return next(err); }
        // Success, so redirect to bookinstances
        res.redirect('/catalog/bookinstances');
    });
};
